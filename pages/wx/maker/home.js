...
Page({
	...
	getDomTop(e) {
		...
		var ele = e.currentTarget.dataset.ele,
			eleName = e.currentTarget.dataset.ele;
		if (ele) {
			ele = '#' + ele;
			var itemList = [];
			const query = wx.createSelectorQuery(),
			...
			query.exec(function(res) {
				...
				var top = res[0].top - 10 + (e.detail.height * radio)
				that.setData({
					delTopHeight:top
				})
				var i = {
	...
	onLoad: function(options) {
		if(typeof(options.currentItem) != 'undefined'){
			this.setData({
	...
	getImgList() {
		...
		wx.request({
			...
			success: function(res) {
				...
				var currentData = list.find(val =>{
					return val.index == that.data.currentItem
				})
				if(!currentData){
					currentData = list[0]
				}
